...
function users_registerNew()
{
    ...
    $newUserByName = users_load(['userName' => params_get('userName', '')]);
    $newUserByEmail = users_load(['email' => params_get('email', '')]);
...
function users_logMeIn()
{
    ...
    if (strpos($userName, '@') > 0) {
        ...
        $user = users_load(['email' => $userName]);
    } else {
        grace_debug('username based login');
        $user = users_load(['userName' => $userName]);
    }
    ...
    if (password_verify(params_get('pwd', ''), users_deshash($user->pwd))) {
        ...
        return ['sessionKey' => users_generateSessionKey($user->idUser), 'userName' => $user->userName, 'idUser' => $user->idUser];
    } elseif ($user->pwd === md5_hash(params_get('pwd', ''))) {
        // Create a token
        grace_debug('Able to login');
...
function users_updateProfile()
{
    ...
    if ($user->userName != $dets['userName']) {
        $newUserByName = users_load(['userName' => $dets['userName']]);
    ...
    if ($user->email != $dets['email']) {
        ...
        $newUserByEmail = users_load(['email' => $dets['email']]);
