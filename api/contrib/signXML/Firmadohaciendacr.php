...
class Firmadocr
{
    ...
    public function retC14DigestSha1($strcadena, $exclusive = false)
    {
        $strcadena = str_replace("\r", '', str_replace("\n", '', $strcadena));
        $d1p = new DOMDocument('1.0', 'UTF-8');
        $d1p->loadXML($strcadena);
        $strcadena = $d1p->C14N($exclusive, false);
    ...
    {
        error_log('DEBUG: FIRMANDO HACIENDA CR LOADED - ID: '.$this->signatureID); // Direct output to stderr/log
        ...
        $this->signatureID = 'Signature-'.uniqid();
        $this->signatureValue = 'SignatureValue-'.uniqid();
        $this->XadesObjectId = 'XadesObjectId-'.uniqid();
        $this->KeyInfoId = 'KeyInfoId-'.$this->signatureID;

        $this->Reference0Id = 'Reference-'.uniqid();
    ...
    public function insertaFirma($xml)
    {
        ...
        $d->loadXML($xml);
        $canonizadoreal = $d->C14N(true, false);
        ...
        if (strpos($certData['serialNumber'], '0x') === false) {
            ...
            $serialNumber = $this->stringHex2StringDec($certData['serialNumber']);
        }
      ...
      '</xades:SignedProperties>';
        ...
        $propDigest = $this->retC14DigestSha1($aconop, true);

        $keyinfo_para_hash1 = str_replace('<ds:KeyInfo', '<ds:KeyInfo '.$xmlns_keyinfo, $kInfo);
        $kInfoDigest = $this->retC14DigestSha1($keyinfo_para_hash1, true);
        ...
        $signaturePayload = $d1p->C14N(true, false);
